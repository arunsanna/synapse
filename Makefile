.PHONY: help deploy deploy-infra deploy-gateway-secrets deploy-terminal-feed-bus deploy-embed deploy-gateway deploy-tts deploy-phase1 \
			deploy-llm deploy-stt deploy-speaker deploy-audio \
			release-gateway-remote \
			build-gateway test-health test-embed test-tts logs logs-embed logs-llm \
			logs-gateway logs-feed-bus logs-tts logs-stt logs-speaker logs-audio validate clean \
			status show-routes
...
REMOTE_DIR ?=
GATEWAY_REMOTE_TAG ?=
GATEWAY_SECRETS_FILE ?= manifests/examples/gateway-secrets.example.yaml
...
release-gateway-remote: ## Build/push gateway on Forge and deploy pinned digest
	FORGE_HOST='$(FORGE_HOST)' NAMESPACE='$(NAMESPACE)' IMAGE_REPO='$(REGISTRY)/synapse-gateway' REMOTE_DIR='$(REMOTE_DIR)' TAG='$(GATEWAY_REMOTE_TAG)' ./scripts/release-gateway-remote.sh
...
deploy-gateway-secrets: ## Apply gateway secrets file (must replace placeholder token first)
	@grep -q "CHANGE_ME_TO_A_LONG_RANDOM_TOKEN" $(GATEWAY_SECRETS_FILE) && { \
		echo "ERROR: update dashboard_access_token in $(GATEWAY_SECRETS_FILE) before applying"; \
		exit 1; \
	} || true
	$(KUBECTL) apply -f $(GATEWAY_SECRETS_FILE)

deploy-terminal-feed-bus: ## Deploy Redis bus for multi-replica terminal feed
	$(KUBECTL) apply -f manifests/apps/terminal-feed-redis.yaml
...
deploy-phase1: deploy-infra deploy-gateway-secrets deploy-terminal-feed-bus deploy-embed deploy-tts deploy-gateway ## Deploy Phase 1 (gateway + embeddings + TTS)
	@echo ""
...
deploy: deploy-infra deploy-gateway-secrets ## Deploy all services
...
logs-feed-bus: ## Tail terminal feed Redis bus logs
	$(KUBECTL) -n $(NAMESPACE) logs -f deploy/synapse-terminal-feed-redis
